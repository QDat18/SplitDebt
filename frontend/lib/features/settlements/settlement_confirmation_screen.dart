...
class SettlementConfirmationScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SettlementConfirmationScreen> createState() => _SettlementConfirmationScreenState();
}

class _SettlementConfirmationScreenState extends ConsumerState<SettlementConfirmationScreen>
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Column(
                children: [
                  CircleAvatar(radius: 28, backgroundImage: NetworkImage(widget.fromAvatar)),
                  const SizedBox(height: 6),
                  Text(widget.fromName, style: AppTypography.title.copyWith(fontSize: 14)),
              ...
              Column(
                children: [
                  const Icon(Icons.east_rounded, color: AppColors.p500, size: 28),
                  ...
                  Container(
                    ...
                    child: Text('Tối ưu nợ', style: AppTypography.caption.copyWith(color: AppColors.p700, fontSize: 10, fontWeight: FontWeight.w700)),
                  ),
                ],
              ),
              ...
              Column(
                children: [
                  CircleAvatar(radius: 28, backgroundImage: NetworkImage(widget.toAvatar)),
                  const SizedBox(height: 6),
                  Text(widget.toName, style: AppTypography.title.copyWith(fontSize: 14)),
          ...
          Text('SỐ TIỀN THANH TOÁN', style: AppTypography.caption.copyWith(letterSpacing: 1.2, fontWeight: FontWeight.w700, color: AppColors.n500)),
          const SizedBox(height: 4),
          Text(
            currencyFormatter.format(widget.amount),
            style: AppTypography.display.copyWith(fontSize: 32, fontWeight: FontWeight.w900, color: AppColors.p500),
          ),
        ],
      ),
    );
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Text('Quét mã VietQR chuyển khoản', style: AppTypography.label.copyWith(fontSize: 14)),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(color: AppColors.infoTint, borderRadius: AppDimensions.radius8),
                child: Text('MBBank', style: AppTypography.caption.copyWith(color: AppColors.info, fontWeight: FontWeight.w700)),
              ),
            ],
          ),
          const SizedBox(height: AppDimensions.s16),
          ...
          Container(
            ...
            child: Column(
              children: [
                const Icon(Icons.qr_code_2_rounded, size: 160, color: AppColors.n900),
                const SizedBox(height: AppDimensions.s8),
                Text('STK: 190368889999 • MBBank', style: AppTypography.bodySmall.copyWith(fontWeight: FontWeight.w700)),
                Text('Chủ TK: ${widget.toName.toUpperCase()}', style: AppTypography.caption),
                const SizedBox(height: 4),
                SelectableText(
                  'Nội dung: ${widget.fromName} chuyen tien SplitDebt',
                  style: AppTypography.caption.copyWith(color: AppColors.p700, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.fact_check_rounded, color: AppColors.p500, size: 20),
              const SizedBox(width: AppDimensions.s8),
              Text('Tiến trình xác nhận 2 chiều (BE2-SET-02)', style: AppTypography.label.copyWith(fontSize: 14)),
            ],
          ),
          const SizedBox(height: AppDimensions.s16),
          ...
          _buildStepRow(
            ...
            subtitle: _payerConfirmed ? 'Đã bấm "Tôi đã chuyển khoản"' : 'Chưa bấm xác nhận đã chuyển',
            isDone: _payerConfirmed,
          ),

          const Padding(
            padding: EdgeInsets.only(left: 15),
            child: SizedBox(height: 20, child: VerticalDivider(thickness: 2, color: AppColors.n200)),
          ),
          ...
          _buildStepRow(
            ...
            subtitle: _receiverConfirmed ? 'Đã duyệt nhận đủ tiền -> Chốt sổ!' : 'Đang chờ người nhận duyệt tiền',
      ...
      children: [
        CircleAvatar(
          ...
          child: isDone
              ...
              : Text(stepNumber, style: AppTypography.caption.copyWith(fontWeight: FontWeight.w700, color: AppColors.n700)),
        ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(subtitle, style: AppTypography.caption.copyWith(color: isDone ? AppColors.success : AppColors.n500)),
            ],
          ),
        ),
      ],
            ...
            if (!isFullyCompleted) ...[
              if (!_payerConfirmed)
                ElevatedButton.icon(
                  ...
                  label: Text('Bước 1: Tôi đã chuyển khoản', style: AppTypography.title.copyWith(color: Colors.white, fontSize: 15)),
                ),
              ...
              if (_payerConfirmed && !_receiverConfirmed)
                ElevatedButton.icon(
                  ...
                  icon: const Icon(Icons.verified_rounded, size: 20, color: Colors.white),
                  label: Text('Bước 2: Người nhận bấm "Đã nhận tiền"', style: AppTypography.title.copyWith(color: Colors.white, fontSize: 15)),
                ),
            ] else ...[
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.task_alt_rounded, color: AppColors.success, size: 24),
                    const SizedBox(width: AppDimensions.s8),
                    Text('GIAO DỊCH ĐÃ THANH TOÁN THÀNH CÔNG!', style: AppTypography.title.copyWith(color: AppColors.success, fontSize: 14)),
                  ],
                ),
              ),
              ElevatedButton.icon(
